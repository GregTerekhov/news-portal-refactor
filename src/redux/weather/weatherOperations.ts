  ...
  async (position, { rejectWithValue }) => {
    ...
    try {
      const response = await axios.get<WeatherData>(
        `${CONFIG.BASE_URL_WEATHER}/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=${API_KEY}`,
      );
  ...
  async (position, { rejectWithValue }) => {
    ...
    try {
      const response = await axios.get(
        `${CONFIG.BASE_URL_WEATHER}/data/2.5/forecast?lat=${latitude}&lon=${longitude}&cnt=6&units=metric&appid=${API_KEY}`,
      );
